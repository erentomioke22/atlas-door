...
export const productValidation =  yup.object().shape({
    ...
    colors: yup.array(
      yup.object({
        name: yup.string().min(1, 'Color name is required').required("name field must not be empty"),
        hexCode: yup.string().min(1, 'Hex code is required').required("hexcode field must not be empty"),
        price: yup.number().min(0.01, 'Price must be greater than 0').required("price field must not be empty"),
        stocks: yup.number().min(1, 'stocks must be greater than 0').required("stocks field must not be empty"),
        discount: yup.number().min(1, 'stocks must be greater than 0').max(100,'discount must be less than 100').required("discount field must not be empty"),
        // imageUrl: yup.string().url('Invalid URL').min(1, 'Image URL is required')
      })
    ).min(1, 'At least one color is required').required("color field must not be empty"),
    // price:yup.number().required("price field must not be empty"),
    // count:yup.number().required("count field must not be empty"),
    // discount:yup.number()
});
