...
export const postValidation =  yup.object().shape({
    title: yup.string().trim().strict(true).min(5, "Title must be at least 5 character").max(100, "Title cannot exceed 100 characters").required("Title field must not be empty"),
...
export const orderGatewayValidation =  yup.object().shape({
    user: yup.string().trim('نباید ابتدا و انتها متن فاصله باشد').strict(true).min(8, "نام و نام خانوادگی نباید کمتر از ۸ حرف باشد").max(30, "نام و نام خانوادگی نباید بیشتر از ۳۰ حرف باشد").required("Title field must not be empty"),
    ...
    phone: yup.string()
    .matches(/^[0-9]+$/, "فقط باید حاوی اعداد باشد")
    .min(10, "شماره تماس حداقل باید ۱۰ عدد باشد")
    .max(15, "شماره تماس نباید بیشتر از ۱۵ عدد باشد")
    .required("فیلد شماره تماس نباید خالی باشد"),
    address: yup.string().trim('نباید ابتدا و انتها متن فاصله باشد').strict(true).min(10, "آدرس نباید کمتر از ۱۰ کاراکتر باشد").max(512, "آدرس نباید بیشتر از ۵۱۲ کاراکتر باشد").required("فیلد آدرس نباید خالی باشد"),
...
export const imageFileValidation =  yup.object().shape({
    image: yup.mixed().required("Image field must not be empty").test("fileSize", "The file is too large", value => value && value.size < 5000000).test("fileType", "Unsupported file type", value =>  value && ["image/jpeg","image/jpg", "image/png", "image/webp"].includes(value.type)),
});
...
export const settingProfileValidation =yup.object().shape({
    ...
    address: yup.string().max(200,"آدرس نباید بیشتر از ۲۰۰ کاراکتر باشد"),
    // image: yup.string(),
