...
const CreateProduct = () => {
  ...
  const handleAddColor = () => {
    if (editColorIndex !== null) {
      const updatedColor = colors.map((color, index) =>
        index === editColorIndex ? { colorName, colorHex,colorPrice,colorDiscount,colorStocks } : color
      );
      setColors(updatedColor);
      setValue("colors", updatedColor, { shouldValidate: true });
      setEditColorIndex(null);
    } else {
      setColors([...colors, { colorName, colorHex,colorPrice,colorDiscount,colorStocks }]);
      setValue("colors", [...colors, { colorName, colorHex,colorPrice,colorDiscount,colorStocks }], {
        shouldValidate: true,
      });
    }
    setColorHex("");
  ...
  const handleEditColor = (index) => {
    ...
    setColorHex(color.colorHex);
    setColorPrice(color.colorPrice);
      ...
      <div className="w-2/3 flex flex-wrap gap-2 mx-auto items-center my-5">
        {colors?.map((color, index) => (
          <div key={index} className=" bg-lcard dark:bg-dcard p-2 rounded-xl">
            <div className="flex gap-2">
                  ...
                  />
                   <div className="flex flex-col text-sm">
                      <p>{color.colorName}</p>
                      <p className="line-through text-red decoration-red">{formatPrice(color.colorPrice)}</p>
                      <p>{formatPrice(color.colorPrice * (1- color.colorDiscount/100))}</p>
                   </div>
            ...
            <div>
              <p>{color.colorDiscount}</p>
              <p>{color.colorStocks}</p>
            </div>
          </div>
        ))}
      </div>
