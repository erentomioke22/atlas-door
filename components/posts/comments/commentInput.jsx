...
const CommentInput = ({post,title,header,content,commentId,reply,edit,replyId,placeHolder,btnStyle}) => {
  ...
  // console.log(values)
    ...
    name:session?.user.displayname || values.name,
    userId:values.userId,
    commentId,
    image:values.image
    },{
  ...
  return (
    ...
    btnStyle={btnStyle}
    onClose={onClose}
    className={"right-0 bg-white dark:bg-black dark:border-dbtn border border-lbtn px-3 w-72  shadow-lg"}
>
          <form onSubmit={handleSubmit(onSubmit)} className='space-y-3 text-right'>
          {session &&  session?.user.role === 'admin' || 
          <>  
          <input className="block  w-full p-2 text-sm bg-lcard dark:bg-dcard focus:outline-none focus:ring-2 rounded-lg duration-200 focus:ring-black dark:focus:ring-white text-right" type="text"  placeholder='نام ' {...register("name")} name="name"/>
              <div
          ...
          <input className="block  w-full p-2 text-sm bg-lcard dark:bg-dcard focus:outline-none focus:ring-2 rounded-lg duration-200 focus:ring-black dark:focus:ring-white text-right" type="email" placeholder='ایمیل' {...register("email")}  name="email"/>
          <div
                className={`text-red mt-2  text-[10px] md:text-sm transition-opacity duration-300  ${
             ...
             <Controller
                  ...
                  control={control}
                  render={({ field }) => (
                    <CommentTextEditor content={field.value} onChange={field.onChange} ref={field.ref} title={`WRITE YOUR ${placeHolder} HERE ...`}/>
                  )}
               />
              <div
                className={`text-red mt-2  text-[10px] md:text-sm transition-opacity duration-300  ${
