  ...
  const uploadMutation = useUploadMutation();
    ...
    return (     
      ...
      <div className="flex gap-2  my-3">
        <div>
        <button type="button" onClick={()=>{actions.handleAdd; setOpen(!open)}} className="bg-black  dark:bg-white text-white dark:text-black p-2 rounded-full">
          {open? <Icon name="Plus"/> : <Icon name="Plus"/>}
        </button>
        </div>
     <div className={`transition-transform duration-500  transform ${open ? "max-w-fit flex flex-wrap gap-2" :"max-w-0 opacity-0 hidden"}`}>
      <Dropdown title={<Icon name="Video"/>}  className={"-left-14 w-72 z-50  overflow-auto bg-white border border-lbtn px-3 dark:border-dbtn dark:bg-black "}
         btnStyle={"p-2 rounded-full border-2 border-black dark:border-white text-black dark:text-white"}>
      ...
      <Dropdown title={<Icon name="Image"/>}  className={"-left-24 w-72 z-50  overflow-auto bg-white border border-lbtn px-3 dark:border-dbtn dark:bg-black "}
         btnStyle={"p-2 rounded-full border-2 border-black dark:border-white text-black dark:text-white"}>
<input
    type='file'
    accept='image/*'
