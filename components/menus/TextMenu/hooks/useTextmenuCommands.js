...
export const useTextmenuCommands = (editor) => {
  ...
  const onHeading1 = useCallback(() => editor.chain().focus().toggleHeading({ level: 1 }).run(), [editor])
  const onHeading2 = useCallback(() => editor.chain().focus().toggleHeading({ level: 2 }).run(), [editor])
  const onHeading3 = useCallback(() => editor.chain().focus().toggleHeading({ level: 3 }).run(), [editor])
  ...
  // const onChangeColor = useCallback((co) => editor.chain().setColor(color).run(), [editor])
  const onChangeColor = useCallback(() => editor.chain().setColor('#958DF1').run(), [editor])
  const onClearColor = useCallback(() => editor.chain().focus().unsetColor().run(), [editor])

  const onChangeHighlight = useCallback((co) => editor.chain().toggleHighlight().run(), [editor])
  // const onChangeHighlight = useCallback((co) => editor.chain().setHighlight({ color }).run(), [editor])
  ...
  const onTone = useCallback(
    (tone) => editor.chain().focus().aiAdjustTone(tone, { stream: true, format: 'rich-text' }).run(),
    [editor],
  )
  const onTranslate = useCallback(
    (language) => editor.chain().focus().aiTranslate(language, { stream: true, format: 'rich-text' }).run(),
    [editor],
  )
