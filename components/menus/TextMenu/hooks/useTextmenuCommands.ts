...
export const useTextmenuCommands = (editor: Editor) => {
  ...
  const onChangeColor = useCallback((color: string) => editor.chain().setColor('#958DF1').run(), [editor])
  ...
  const onChangeHighlight = useCallback((color: string) => editor.chain().toggleHighlight().run(), [editor])
