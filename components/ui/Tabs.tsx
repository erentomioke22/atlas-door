...
type Variant = 'underline' | 'pills' | 'cards'
...
export default function Tabs({ tabs, defaultIndex = 0, variant = 'underline', className = '' }: TabsProps) {
  ...
  return (
    <div className={` ${className}`}>
      <div role="tablist" aria-label="Tabs navigation" className={`flex ${styles.tabList}`}>
        {tabs.map((tab, index) => (
          <button
            key={index}
            role="tab"
            ...
            className={`relative flex items-center space-x-2 focus:outline-none focus:ring-0 ring-0 text-center mx-auto ${styles.tab(activeTab === index)} ${tab.disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
            {tab.icon && <span>{tab.icon}</span>}
            <span className="text-center mx-auto">{tab.label}</span>
            {variant === 'underline' && activeTab === index && <span className={styles.activeIndicator} />}
          </button>
        ))}
      </div>
        ...
        {tabs.map((tab, index) => (
          <div
            ...
            className={`transition-all duration-500 ease-in-out ${
              activeTab === index ? 'opacity-100 transform translate-y-0' : 'opacity-0 absolute transform -translate-y-2 h-0 overflow-hidden'
            }`}
          >
            <div className={activeTab === index ? 'animate-fadeIn' : ''}>{tab.content}</div>
          </div>
        ))}
      </div>
    </div>
