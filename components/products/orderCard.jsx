...
const OrderCard = ({ item }) => {
  ...
  const discountedPrice = item?.color.price - (item?.color.price * (item?.color?.discount || 0)) / 100;
  ...
  return (
    ...
    <div className='flex flex-wrap justify-between gap-2 text-sm'>
         <p>مجموع قيمت كالا<span className='text-[10px]'>(تعداد * قيمت هر واحد)</span> </p>
            <div>
              <p>{formatPrice(totalItemPrice)} تومان</p>
            </div>
      </div>
    </div>
  )
