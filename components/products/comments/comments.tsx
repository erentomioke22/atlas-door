  ...
  return (
    <Offcanvas
      ...
      {status === "error" || (data as any)?.error || error
        ? <p className="text-center  text-sm text-neutral-500 dark:text-neutral-300">مشكلي در برقراري ارتباط وجود دارد</p>
        : <div>
              ...
              {status === "pending" ? (
                ...
                <InfiniteScrollContainer className={''} onBottomReached={() => hasNextPage && !isFetching && fetchNextPage()}>
                  {status === "success" && !comments.length  && !hasNextPage &&
                    <p className="text-center  text-sm text-neutral-500 dark:text-neutral-300">هنوز بازخوردي اينجا قرار نگرفته</p>
                  }
                ...
                : <div className="sticky -bottom-5 left-0 right-0 bg-white dark:bg-black py-5">
                    {commentId &&
                      <div className="flex justify-between gap-3">
                        ...
                        <div className="my-auto">
                          <button aria-label="close button" title="close button" className="bg-lcard dark:bg-dcard px-2 py-1 rounded-full border-2 text-neutral-500 dark:text-neutral-300" onClick={()=>{setCommentId(null) ; setMessage(null);setReplyInfo(null)}}><IoClose/></button>
                        </div>
                      </div>
                    }
                    <div className="flex gap-2">
                      {session?.user?.image === null
                        ...
                        : 
                            <ImageCom
                              className="rounded-xl  inset-0 w-10 h-10"
