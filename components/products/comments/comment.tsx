...
const Comment: React.FC<CommentProps> = ({
  content,user,createdAt,userId,id,replies,likes,_count,parent,product,margin,category,scrollToComment,setCurrentCommentId,setMessage,setReplyInfo,session
}) => {
  ...
  return (
    <div className="comment w-full  space-y-3 py-5" id={id}>
      ...
      <div className="text-[10px] md:text-[12px] ">
        {content?.length >= 251 ? (
          <>
            {ShowAll === id ? (
              <>
                <div className=" break-words w-full  normal-case leading-loose  text-sm  " dangerouslySetInnerHTML={{ __html:content }}/> {" "}
              ...
              <>
                <div className=" break-words w-full  normal-case leading-loose  text-sm  " dangerouslySetInnerHTML={{ __html:content.slice(0, 250) }}/> 
      ...
      <div className="  ">
        <button aria-label="reply button"  title="reply button"onClick={()=>{setCurrentCommentId(id); setMessage(null); setReplyInfo({user: user as any, content})}}><LuReply/></button>
      </div>
    </div>
  );
};
