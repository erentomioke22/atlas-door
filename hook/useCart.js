...
//   // Sync with server when adding items
...
//   // Sync with server when updating quantity
...
//   // Sync with server when removing items
...
export const useCart = () => {
  ...
  const { mutate: syncAddToServer } = useMutation({
    mutationFn: async ({ productId, colorId }) => {
      return axios.post(`/api/product/addToBag?productId=${productId}&colorId=${colorId}`);
    },
    onSuccess: () => {
      // Remove from pending sync on success
      setPendingSync(prev => prev.filter(sync => 
        !(sync.type === 'add' && sync.data.productId === productId && sync.data.colorId === colorId)
      ));
    },
  ...
  const { mutate: syncUpdateToServer } = useMutation({
    mutationFn: async ({ productId, colorId, quantity }) => {
      return axios.put(`/api/product/addToBag?productId=${productId}&colorId=${colorId}`, 
        { quantity }
      );
    },
    onSuccess: () => {
      setPendingSync(prev => prev.filter(sync => 
        !(sync.type === 'update' && sync.data.productId === productId && sync.data.colorId === colorId)
      ));
    },
  ...
  const { mutate: syncRemoveFromServer } = useMutation({
    ...
    onSuccess: () => {
      setPendingSync(prev => prev.filter(sync => 
        !(sync.type === 'remove' && sync.data.productId === productId && sync.data.colorId === colorId)
      ));
    },
    onError: (error, { productId, colorId }) => {
